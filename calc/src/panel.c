	...
	if (pl)
	{
		if (pl->_front)
		{
			...
			while (pn)
			{
				...
				{
					FillRect(hdc, &pRect, (HBRUSH)(COLOR_WINDOWFRAME));
...
GeneralPanel* GeneralPanel_init(const wchar_t* inStr, const wchar_t* outStr)
{
	...
	GList_pushback(p->_in_gitems_list, (GItem*)GItemChar_init(PROC_LITERAL, 0));
...
Draw2DPanel* Draw2DPanel_init(const wchar_t* funcStr, const wchar_t* intervalStr, const wchar_t* stepStr)
{
	...
	GList_pushback(p->_func_gitems_list, (GItem*)GItemChar_init(PROC_LITERAL, 0));
	GList_pushback(p->_interval_gitems_list, (GItem*)GItemChar_init(PROC_LITERAL, 0));
	GList_pushback(p->_step_gitems_list, (GItem*)GItemChar_init(PROC_LITERAL, 0));
